...
The following are the collectors we've included as part of the package, together with
all of the metric names they report on and what they mean. If you have any others you'd
like to contribute, we'd love to hear about them so we can reference them or include
them with your permission in a future release.
...
These stats are from ``/proc/net/dev`` that provides network interface statistics
...
See `iostats.txt <http://www.kernel.org/doc/Documentation/iostats.txt>`_ kernel specific metrics, depending on if you have a 2.6 kernel before 2.6.25 or after.

``/proc/diskstats`` has stats for a given physical device.
...
Metrics from ``/proc/net/sockstat`` (socket allocation).
