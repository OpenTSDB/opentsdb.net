...
The lookup command is the default for ``uid`` used to lookup the UID assigned to a name or the name assigned to a UID for a given type.
...
.. csv-table::
   ...
   "Forward metrics mapping is missing reverse mapping: foo -> 000001", "This may occur if a TSD crashes before the reverse map is written and would only prevent queries from executing against time series using the UID as they would not be able to lookup the name.", "The fix is to restore the missing reverse map."
